#!/usr/bin/env python3
"""
Biometric Document Encryptor - Working Version
Fixed QR generation and decoding issues
"""
...
class BiometricDocumentEncryptor:
    def __init__(self):
        ...
        self.max_qr_size = 1500  # Very conservative QR data limit
    ...
    def create_biometric_key(self, face_roi):
        ...
        hist = cv2.calcHist([face_roi], [0], None, [8], [0, 256])  # Only 8 bins
        hist = hist.flatten()
        ...
        key_data = {
            'h': [round(float(x), 1) for x in hist[:6]],  # Only 6 histogram values
    ...
    def encrypt_document(self, file_path, biometric_signature):
        ...
        try:
            ...
            metadata = {
                'f': os.path.basename(file_path)[:15],  # Truncate long filenames
                's': len(document_data),
                't': int(time.time())
            }
    ...
    def create_data_qr_series(self, encrypted_data, base_filename):
        ...
        chunk_size = self.max_qr_size - 50  # Buffer for metadata
    ...
    def decrypt_from_qr_system(self, key_qr_file, data_qr_files, output_dir="decrypted"):
        ...
        for data_qr_file in data_qr_files:
            ...
            if image is None:
                print(f"❌ Cannot read {data_qr_file}")
                continue
            
            decoded_objects = pyzbar.decode(image)
            if not decoded_objects:
                print(f"❌ No QR code found in {data_qr_file}")
                continue
...
def main():
    print("🌊⚡ BIOMETRIC DOCUMENT ENCRYPTOR - WORKING VERSION ⚡🌊")
    print("=" * 65)
